...
so a production deploy never silently runs against a mock or an
unintended host. See `docs/frontend-env-vars.md` for the full reference.

---

## Contributing

Thanks for helping build Mux Dashboard. This section covers local setup,
the branch/PR workflow, commit conventions, and the security and testing
...
1. Fork the repo and clone your fork:
   ...
   ```bash
   git clone https://github.com/<your-username>/mux-frontend.git
   cd mux-frontend
   ```

2. Install dependencies with pnpm (the repo is pnpm-only; do not mix in
   npm/yarn lockfiles):
...
3. Copy `.env.example` to `.env.local` and fill in values as needed. All
   variables are optional locally — mock/default behavior kicks in when
   they're unset. See [Environment variables](#environment-variables) above
   and `docs/frontend-env-vars.md` for the full reference.
...
* Branch off `main` with a descriptive name, e.g.
  `feat/wallet-export` or `fix/network-scope`.
* Keep PRs focused: one issue per PR, minimal unrelated churn.
* Reference the issue in the PR description (`Closes #<issue>`), and
  include a short design note stating the invariants your change relies on
  and how it fails closed.
* Ensure CI is green before requesting review. Required checks are defined
  in `.github/workflows/ci.yml`; see `docs/ci-typecheck.md` for the
  typecheck gate.
* For any money-path or mainnet-affecting change, land it behind a feature
  flag or kill-switch and document the rollback strategy in the PR
  description.
...
subject imperative and under ~72 characters; put rationale and edge cases
in the body.

### Security expectations

* **No secrets in the repo or logs.** Never commit API keys, private keys,
  JWTs, or webhook secrets. Redact key material, tokens, and secrets from
  logs and error messages. Server-only credentials (`MUX_API_KEY`,
  `MUX_API_SECRET`) must never be prefixed with `NEXT_PUBLIC_` or reach the
  browser.
* **Deny-by-default for privileged surfaces.** Every new privileged
  entrypoint (API route, admin action, delegate/guardian flow) must be
  authorized explicitly — owner/delegate/guardian/API-key/JWT — and must
  fail closed when auth is missing, expired, or the role is wrong.
* **Server/contract is the source of truth** for spends, recovery, and
  admin. The frontend never persists or fabricates money-path state; it
  proxies to the backend and surfaces stable error codes with correlation
  ids. See `docs/security-ux-guards.md` for the UX guardrails that back
  this up.
* **Fail closed on dependency outages.** When the backend/RPC/Horizon is
  unreachable, writes must not proceed on stale or fabricated data.

### Testing expectations

* **Unit tests** (Vitest) cover invariants and auth negatives. Run them
  with `pnpm test`.
* **E2E tests** live under `tests/e2e/` — see `tests/e2e/README.md` for how
  to run them.
* **Real-backend E2E** exercises the critical path against a live backend;
  see `docs/e2e-real-backend-testing.md` for setup and the manual checklist
  used where automation can't cover wallet/RPC flows.
* **CI** must stay green for the package you touch. If a check is ungated,
  add it as a required check rather than relying on reviewers to run it.

### Related docs

* `docs/security-ux-guards.md` — security UX guardrails
* `docs/frontend-env-vars.md` — environment variable reference
* `docs/ci-typecheck.md` — typecheck CI gate
* `docs/e2e-real-backend-testing.md` — real-backend E2E testing
* `tests/e2e/README.md` — E2E suite overview
